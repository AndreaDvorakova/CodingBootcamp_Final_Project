...
export default function DetailPage() {
    ...
    const changeSelect = (e) => {
      ...
      return (
        <>
        <select id="selectCity" value={selectedCity} onChange={changeSelect}>
           {pharmacyCities.map((city) => {
             return (<option value={city}>{city}</option>)
           })}
        </select>
        {
            pharmacyItems.filter((pharmacyItem) => (selectedCity == pharmacyItem.pharmacy.city || selectedCity == "all")).map((pharmacyItem) => {
                return (
